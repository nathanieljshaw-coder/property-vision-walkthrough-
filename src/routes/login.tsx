...
export const Route = createFileRoute("/login")({
  head: () => ({
    meta: [{ title: "Login | LUMEN Client Dashboard" }],
  }),
  component: LoginPage,
});
      ...
      <div className="w-full max-w-md">
        <div className="text-center">
          ...
          <p className="mt-2 text-sm text-muted-foreground">
            Sign in to view your orders and walkthroughs.
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="mt-8 space-y-5">
          ...
          <div>
            ...
            <p className="mt-1.5 text-xs text-muted-foreground/60">
              {email === "nathaniel.j.shaw@outlook.com" ? (
                <>Admin sign-in — password optional.</>
              ) : (
                <>Leave blank only for the admin account.</>
              )}
            </p>
            <div className="mt-2 text-right">
              <Link to="/forgot-password" className="text-xs font-medium text-gold/70 underline underline-offset-4 transition-colors hover:text-gold">
                Forgot password?
              </Link>
            </div>
          </div>
        ...
        <p className="mt-6 text-center text-sm text-muted-foreground">
          ...
          <Link to="/register" className="font-medium text-gold underline underline-offset-4 transition-colors hover:text-gold/80">
            Create one
          </Link>
        </p>
      </div>
