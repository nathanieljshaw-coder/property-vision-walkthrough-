#!/bin/bash
# Builds the home-page hero background montage: all six portfolio walkthroughs
# stitched into one looping 1280x720 video with crossfades (6 x 30s - 5 x 0.4s
# fades = ~178s). Referenced from src/routes/index.tsx as /walkthroughs/hero-montage.mp4.
...
"$FFMPEG" -y \
  ...
  " \
  -map "[vout]" -c:v libx264 -preset faster -crf 25 -pix_fmt yuv420p \
